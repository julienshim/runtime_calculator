let grid = [[08,02,22,97,38,15,00,40,00,75,04,05,07,78,52,12,50,77,91,08], // 0
...
function h (set) {
  let max = 0;
  set.forEach(function(test){
    for (let i = 0; i < test.length - 5; i++) {
      let four = test[i] * test[i+1] * test[i+2] * test[i+3];
      if (four > max) {
        max = four;
      }
    }
...
function d1 (set) {
  let max = 0;
  let v = set[0].length - 5;
  let h = set.length - 5;
  for (let i = 0; i < v; i++) {
    for (let j = 0; j < h; j++) {
      let four = set[i][j] * set[i+1][j+1] * set[i+2][j+2] * set[i+3][j+3];
      if (four > max) {
        max = four;
      }
    }
...
function d2 (set) {
  ...
  for (let i = 0; i < v; i++) {
    for (let j = 0; j < h; j++) {
      let four = set[i][j+3] * set[i+1][j+2] * set[i+2][j+1] * set[i+3][j];
      if (four > max) {
        max = four;
      }
    }
...
function v (set) {
  ...
  for (let i = 0; i < v; i++) {
    for (let j = 0; j < h; j++) {
      let four = set[i][j] * set[i+1][j] * set[i+2][j] * set[i+3][j];
      if (four > max) {
        max = four;
      }
    }
